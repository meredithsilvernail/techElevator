...
JOIN film_actor ON actor.actor_id = film_actor.actor_id
JOIN film ON film.film_id = film_actor.film_id
WHERE actor.last_name = 'DEAN' AND actor.first_name IN('RIVER', 'JUDY');
...
SELECT (customer.last_name || ', ' || customer.first_name) AS name, COUNT(rental.rental_id) AS total_rentals
FROM customer
...
SELECT (customer.last_name || ', ' || customer.first_name) AS name, SUM(payment.amount) AS dollars_spent
FROM customer
...
SELECT (actor.first_name || ' ' || actor.last_name) AS actor_name, COUNT(rental.*) AS total_rentals
FROM actor
JOIN film_actor ON actor.actor_id = film_actor.actor_id
JOIN film ON film.film_id = film_actor.film_id
JOIN inventory ON film.film_id = inventory.film_id
...
SELECT (actor.first_name || ' ' || actor.last_name) AS actor_name, COUNT(rental.rental_id) AS total_rentals
FROM category
JOIN film_category ON category.category_id = film_category.category_id
JOIN film ON film_category.film_id = film.film_id
