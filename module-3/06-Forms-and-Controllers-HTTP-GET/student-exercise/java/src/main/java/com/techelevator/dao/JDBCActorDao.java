...
public class JDBCActorDao implements ActorDao {
    ...
    public List<Actor> getMatchingActors(String search) {
        ...
        String actorSearchSql = "SELECT first_name, last_name, FROM actor WHERE last_name LIKE ? order by last_name";
        SqlRowSet results = jdbcTemplate.queryForRowSet(actorSearchSql, "%" + search.toUpperCase() + "%");
        while (results.next()) {
